...
export async function answerInterviewQuestion(args: {
  ...
  try {
    ...
    if (!feedback.followUpQuestion && followUpCount < MAX_FOLLOW_UPS_PER_QUESTION) {
      const retryFollowUp = await requestRequiredFollowUpQuestion(
        session,
        question,
        answer,
        followUpCount,
      ...
      if (retryFollowUp?.question) {
        feedback = {
          ...feedback,
          followUpQuestion: retryFollowUp.question,
          followUpNcsAreas: retryFollowUp.ncsAreas,
        };
      }
    }
...
질문은 실제 면접관이 말하듯 자연스럽게 작성하되, 지원자가 대충 답할 수 없도록 압박면접처럼 논리적이고 날카롭게 물어보세요. 단, 무례하거나 비난하는 표현은 피하세요.
각 질문은 너무 짧은 단문으로 끝내지 말고 2~4문장으로 구성하세요. 먼저 공고/직무 상황을 제시하고, 이어서 본인 역할, 판단 기준, 근거 자료, 행동 순서, 결과 중 최소 2가지를 구체적으로 요구하세요.
모든 문장에 기업명이나 직무명을 반복해서 넣지 말고, 필요할 때만 "우리 기관", "우리 병원", "해당 직무", "현장"처럼 실제 면접에서 쓰는 표현으로 맥락을 녹이세요.
...
- 질문은 실제 면접관이 말하듯 자연스럽게 작성하되, 지원자가 대충 답할 수 없도록 압박면접처럼 논리적이고 날카롭게 물어보세요. 단, 무례하거나 비난하는 표현은 피하세요.
- 각 질문은 너무 짧은 단문으로 끝내지 말고 2~4문장으로 구성하세요. 먼저 공고/직무 상황을 제시하고, 이어서 본인 역할, 판단 기준, 근거 자료, 행동 순서, 결과 중 최소 2가지를 구체적으로 요구하세요.
- 기업명과 직무명을 매번 문장에 억지로 넣지 말고, 필요할 때만 "우리 기관", "우리 병원", "해당 직무", "현장"처럼 자연스러운 표현으로 맥락을 녹이세요.
...
아직 ${MAX_FOLLOW_UPS_PER_QUESTION}개가 나오지 않았다면 followUpQuestion은 반드시 비어 있지 않은 문자열로 반환하고, followUpNcsAreas에는 NCS 7개 후보 중 이번 꼬리질문이 검증하는 역량 1~2개를 넣으세요.
지원자의 답변이 짧거나 부족해도 질문을 생략하지 말고, 빠진 판단 근거나 실제 행동을 확인하는 꼬리질문을 만드세요.
...
summary와 nextAnswerGuide에서 목록화가 필요하면 반드시 한 줄에 하나씩 "1. 내용", "2. 내용" 형식으로 작성하고, 번호와 내용을 반드시 같은 줄에 붙여 쓰세요.
"1."처럼 번호만 단독 줄에 두지 마세요. "(1)", "1)", "①" 같은 번호 표기는 사용하지 말고, 한 문장 안에 여러 번호를 붙여 나열하지 마세요.
...
질문과 관련은 있지만 구체적 사례/역할/판단 근거가 부족한 답변은 30~59점 범위에서 차등 평가하세요.
직무 맥락, 본인 역할, 판단 근거, 실행 과정, 결과가 확인되는 답변은 60~100점 범위에서 완성도에 따라 차등 평가하세요.
...
꼬리질문은 지원자의 이번 답변에서 빠진 상황, 본인 역할, 판단 근거, 행동, 결과, 객관적 기준 중 하나 이상을 구체적으로 파고드는 질문이어야 합니다.
압박면접처럼 논리적이고 객관적으로 물어보세요. 단, 무례하거나 비난하는 표현은 피하고 실제 면접관이 할 법한 수준으로 작성하세요.
followUpQuestion은 실제 면접관이 지원자의 답변을 듣고 바로 이어 묻는 말투로 작성하세요. 너무 짧은 단문으로 끝내지 말고 2~4문장으로, 답변의 빈틈을 짚은 뒤 추가로 확인할 근거/순서/결과를 요구하세요.
...
summary, questionReviews.summary, followUpScores.summary, strengths, improvements, futurePracticeQuestions에서 목록화가 필요하면 반드시 한 줄에 하나씩 "1. 내용", "2. 내용" 형식으로 작성하고, 번호와 내용을 반드시 같은 줄에 붙여 쓰세요.
"1."처럼 번호만 단독 줄에 두지 마세요. "(1)", "1)", "①" 같은 번호 표기는 사용하지 말고, 한 문장 안에 여러 번호를 붙여 나열하지 마세요.
목록 앞뒤로 불필요한 괄호를 단독으로 남기지 말고, 문단과 목록 사이에는 자연스럽게 줄바꿈하세요.
...
의미 있는 답변이 부족해도 일반적인 서비스용 100점 척도로 채점하세요.
구간의 시작점이나 중간값으로 고정하지 말고, 답변의 관련성·성실성·구체성·직무 연관성·본인 역할·판단 근거·결과 수준을 종합해 각 구간 안에서 차등 평가하세요.
무응답에 가까운 단답/문맥 무관 답변은 보통 5~29점 범위에서 차등 평가하세요.
질문과 관련은 있지만 근거가 빈약한 답변은 보통 30~59점 범위에서 차등 평가하세요.
구체성과 직무 연관성이 확인되는 답변은 60~100점 범위에서 완성도에 따라 차등 평가하세요.
...
기업/직무: ${session.companyName} / ${session.positionName}
원 질문 관련 NCS: ${question.ncsAreas.join(", ")}
NCS 7개 후보:
${NCS_AREAS.map((area, index) => `${index + 1}. ${area.name}: ${area.description}`).join("\n")}
원 질문: ${question.question}
...
답변이 질문과 관련은 있지만 추상적이면, 사례, 본인 역할, 판단 근거, 실행 과정, 결과 중 빠진 부분을 파고드는 꼬리질문을 만드세요.
지원자의 답변에서 빠진 상황, 본인 역할, 판단 근거, 행동, 결과, 객관적 기준 중 하나 이상을 실제 면접관 말투로 물어보세요.
followUpNcsAreas에는 NCS 7개 후보 중 이번 꼬리질문이 검증하는 역량 1~2개를 넣으세요. 원 질문 관련 NCS를 그대로 복사하지 말고 이번 답변의 빈틈을 기준으로 선택하세요.
압박면접처럼 논리적이고 객관적으로 물어보세요. 단, 무례하거나 비난하는 표현은 피하고 실제 면접관이 할 법한 수준으로 작성하세요.
followUpQuestion은 너무 짧은 단문으로 끝내지 말고 2~4문장으로, 답변의 빈틈을 짚은 뒤 추가로 확인할 근거/순서/결과를 요구하세요.
기업명/직무명을 억지로 반복하지 말고, 필요할 때만 "우리 기관", "우리 병원", "해당 직무", "현장"처럼 자연스럽게 말하세요.
반드시 followUpQuestion에는 비어 있지 않은 한국어 질문 문자열 하나를, followUpNcsAreas에는 NCS 역량 1~2개를 넣은 JSON 객체만 반환하세요.`,
      },
    ],
  });
  ...
  return {
    ...
    ncsAreas: normalizeNcsAreaList(record?.followUpNcsAreas).slice(0, 2),
  };
}
...
function normalizeNcsAreaList(value: unknown) {
  return Array.from(new Set(readStringList(value).map(normalizeNcsAreaName).filter(Boolean))) as NcsAreaName[];
}
...
const requiredFollowUpQuestionSchema = {
  ...
  required: ["followUpQuestion", "followUpNcsAreas"],
  properties: {
    followUpQuestion: { type: "string" },
    followUpNcsAreas: {
      type: "array",
