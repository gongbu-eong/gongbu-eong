...
async function replaceEntries(
  ...
  await Promise.all((payload.activities || []).map((entry, index) => client.query(`INSERT INTO public.user_resume_activities (resume_id, activity_name, description, issuer, activity_date, start_date, end_date, sort_order) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [resumeId, normalizeText(entry.activityName || entry.title) || "활동", normalizeText(entry.description || entry.subtitle), normalizeText(entry.issuer), normalizeText(entry.activityDate), normalizeText(entry.startDate), normalizeText(entry.endDate), index + 1])));
